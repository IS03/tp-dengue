...
Reemplaza la palabra "dr" por "doctor" en la columna B (departamento_nombre).
"""
...
def normalizar_dr_en_archivo(archivo_path):
    """
    Normaliza la columna departamento_nombre reemplazando "dr" por "doctor".
    ...
    for i, fila in enumerate(filas):
        ...
        if len(fila) >= 2:  # Asegurar que hay al menos 2 columnas
            ...
            # Reemplazar la palabra "dr" por "doctor" usando regex para palabras completas
            # \b asegura que sea una palabra completa, no parte de otra palabra
            departamento_modificado = re.sub(r'\bdr\b', 'doctor', departamento_original, flags=re.IGNORECASE)
...
def main():
    ...
    for archivo in archivos_a_procesar:
        try:
            modificaciones = normalizar_dr_en_archivo(str(archivo))
